...
import {
  ...
  AlertDialogHeader,
  AlertDialogTitle,
  AlertDialogOverlay, // IMPORTADO PARA APLICAR CLASSE NO OVERLAY
...
export default function MonthlyClientsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'active':
        return <Badge variant="outline" className="border-green-500 text-green-500 bg-green-50 dark:bg-green-950/20 font-semibold text-xs">✓ Ativo</Badge>;
      case 'suspended':
        return <Badge variant="outline" className="border-orange-500 text-orange-500 bg-orange-50 dark:bg-orange-950/20 font-semibold text-xs">⏸ Suspenso</Badge>;
      case 'inactive':
        return <Badge variant="outline" className="border-gray-500 text-gray-500 bg-gray-50 dark:bg-gray-950/20 font-semibold text-xs">○ Inativo</Badge>;
      default:
        return null;
    }
  };
  ...
  const getPaymentStatusBadge = (status: string) => {
    switch (status) {
      case 'paid':
        return <Badge className="bg-gradient-to-r from-green-500 to-emerald-600 shadow-md text-xs">✓ Pago</Badge>;
      case 'pending':
        return <Badge className="bg-gradient-to-r from-yellow-500 to-orange-600 shadow-md text-xs">⏱ Pendente</Badge>;
      case 'overdue':
        return <Badge className="bg-gradient-to-r from-red-500 to-rose-600 shadow-md animate-pulse text-xs">⚠ Atrasado</Badge>;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 space-y-3 sm:space-y-4 md:space-y-6 pb-20 sm:pb-6">
      ...
      {/* Dialog Editar Agendamentos - usa classes "appointment-modal-*" */}
      <Dialog open={editSchedulesOpen} onOpenChange={setEditSchedulesOpen}>
        <DialogOverlay className="appointment-modal-overlay" />
        <DialogContent className="appointment-modal-content w-[calc(100%-2rem)] sm:max-w-5xl max-h-[90vh] overflow-y-auto rounded-lg">
          {clientToEdit && (
            <>
              <DialogHeader>
      ...
      {/* AlertDialog Cancelar Plano - usa classes "service-completion-modal-*" */}
      <AlertDialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <AlertDialogOverlay className="service-completion-modal-overlay" />
        <AlertDialogContent className="service-completion-modal-content w-[calc(100%-2rem)] max-w-md mx-auto rounded-lg">
          <AlertDialogHeader>
      ...
      {/* AlertDialog Suspender/Reativar Plano - usa classes "service-completion-modal-*" */}
      ...
      {/* Dialog Detalhes do Cliente - usa classes "appointment-modal-*" */}
      <Dialog open={viewDetailsOpen} onOpenChange={setViewDetailsOpen}>
        <DialogOverlay className="appointment-modal-overlay" />
        <DialogContent className="appointment-modal-content w-[calc(100%-1rem)] sm:w-[calc(100%-2rem)] sm:max-w-5xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto rounded-lg p-0">
          {selectedClient && (
            <>
      ...
      {/* Modal Adicionar Cliente (mantém como está; este componente controla seu próprio Dialog) */}
